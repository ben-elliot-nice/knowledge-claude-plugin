/**
 * create-category.js
 * Creates a topic-category page in CXone Expert.
 *
 * Usage: node create-category.js <name> [parent-path]
 *   parent-path defaults to the ROOT_PATH in .env (or "home")
 *
 * Requires .env with: EXPERT_BASE_URL, EXPERT_KEY, EXPERT_SECRET
 * Optional .env:      EXPERT_ROOT_PATH (default parent if not specified)
 */
...
// ─── Args ─────────────────────────────────────────────────────────────────────
const [,, PAGE_NAME, PARENT_PATH = process.env.EXPERT_ROOT_PATH || 'home'] = process.argv;

if (!PAGE_NAME) {
  console.error('Usage: node create-category.js <name> [parent-path]');
  process.exit(1);
}

const ARTICLE_TYPE = 'topic-category';
...
// ─── Main ─────────────────────────────────────────────────────────────────────
async function run() {
  console.log(`[category] "${PAGE_NAME}" under ${PARENT_PATH}`);

  const token  = generateToken();
  const pageId = buildPageId(PARENT_PATH, PAGE_NAME);

  if (await pageExists(pageId, token)) {
    console.warn(`[category] ⚠ Already exists — skipping "${PAGE_NAME}"`);
    return;
  }

  const numericId = await postContents(pageId, PAGE_NAME, TEMPLATE_BODY, token);
  await putTags(numericId, ARTICLE_TYPE, token);
  console.log(`[category] ✓ Created id=${numericId}`);
}

run().catch(err => { console.error('[category] ✗', err.message); process.exit(1); });
